 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */})),cr.addEventListener("click",(async()=>{const t=txtEmail.value,e=txtPassword.value;try{const n=await ti(yp,t,e),i=Lf(vp,"users",n.user.uid);(async()=>{try{await hp(i,{email:t}),console.log("This user has been written to the Firestore")}catch{console.log(`I got an error! ${error}`)}})(),console.log("This user has been written to the database")}catch(t){console.log(`There was an error: ${t}`),mr(t)}})),lr.addEventListener("click",(async()=>{var t;await(t=yp,_t(t).signOut())}));const yp=function(t=ae()){const e=ie(t,"auth");if(e.isInitialized())return e.getImmediate();const n=function(t,e){const n=ie(t,"auth");if(n.isInitialized()){const t=n.getImmediate();if(ft(n.getOptions(),null!=e?e:{}))return t;ke(t,"already-initialized")}return n.initialize({options:e})}(t,{popupRedirectResolver:nr,persistence:[vi,ri,oi]}),i=nt("authTokenSyncURL");if(i){const t=(r=i,async t=>{const e=t&&await t.getIdTokenResult(),n=e&&((new Date).getTime()-Date.parse(e.issuedAtTime))/1e3;if(n&&n>rr)return;const i=null==e?void 0:e.token;sr!==i&&(sr=i,await fetch(r,{method:i?"POST":"DELETE",headers:i?{Authorization:`Bearer ${i}`}:{}}))});!function(t,e,n){_t(t).beforeAuthStateChanged(e,n)}(n,t,(()=>t(n.currentUser))),function(t,e,n,i){_t(t).onIdTokenChanged(e,n,i)}(n,(e=>t(e)))}var r;const s=tt("auth");return s&&Nn(n,`http://${s}`),n}(pp);(async()=>{ei(yp,(t=>{t?(console.log(t),login.style.display="none",app.style.display="block",gr(t),pr()):(fr(),lblAuthState.innerHTML="You're not logged in.")}))})();const vp=function(t,e){const n="string"==typeof t?t:e||"(default)",i=ie("object"==typeof t?t:ae(),"firestore").getImmediate({identifier:n});if(!i._initialized){const t=et("firestore");t&&Af(i,...t)}return i}(pp);
//# sourceMappingURL=index.4826deb6.js.map
