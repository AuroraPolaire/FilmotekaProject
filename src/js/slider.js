import { URL,IMG_URL, API_KEY, poster_path, movie_id, genre_ids, name } from './themoviedb-service';
...
function renderTrendingFilms() {
  ...
  return fetch(trendingUrl).then(response => response.json()).then(({ results }) => {
    return results;
  }).then(renderSliderFilms).catch(error);
}

function filmCardSlider(films) {
  return films.map(({original_title, poster_path, vote_average, movie_id, name}) => {
